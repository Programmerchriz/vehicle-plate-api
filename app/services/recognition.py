...
class RecognitionService:
    ...
    async def recognize_image(
        ...
        try:
            ...
            # save_debug_image("1_original.jpg", image)
            ...
            # save_debug_image("2_plate.jpg", plate_image)

            # Pre-processing
            gray = self.image_processing.to_grayscale(plate_image)

            gray = self.image_processing.resize(gray, width=settings.PLATE_IMAGE_WIDTH)
            # save_debug_image("3_gray.jpg", gray)

            threshold = self.image_processing.adaptive_threshold(gray)
            # save_debug_image("4_threshold.jpg", threshold)

            # OCR
            gray_result, threshold_result = await asyncio.gather(
                asyncio.to_thread(self.ocr.read_text, gray),
                asyncio.to_thread(self.ocr.read_text, threshold),
            )

            # Merge OCR candidates
            all_candidates = (gray_result.candidates + threshold_result.candidates)
            unique_candidates = {}
            ...
            raise RuntimeError("Recognition pipeline failed.") from exc

        processing_time_ms = (time.perf_counter() - start_time) * 1000

        confidence = round((detection.confidence + best_candidate.confidence) / 2, 2)
